...
subroutine micro_mg_tend ( &
  ...
  ! initialize ice property
  real(r8) :: rimtot(mgncol,nlev)            !riming of ice 
  real(r8) :: rim(mgncol,nlev)            !riming of ice 
  real(r8) :: cap_i(mgncol,nlev)         !capacitance of snow 
  real(r8) :: aa_i(mgncol,nlev)    ! snow
  real(r8) :: ba_i(mgncol,nlev)    ! snow
  real(r8) :: am_i(mgncol,nlev)    ! snow
  real(r8) :: bm_i(mgncol,nlev)    ! snow
  real(r8) :: av_i(mgncol,nlev)    ! snow
  real(r8) :: bv_i(mgncol,nlev)    ! snow
  real(r8) :: bmbv1(mgncol,nlev)    ! snow
  real(r8) :: bm1(mgncol,nlev)    ! snow
  real(r8) :: bv1(mgncol,nlev)    ! snow

  !zx add for output
  real(r8) :: nmelttot(mgncol,nlev)        ! melting of cloud ice
  real(r8) :: melt_rain(mgncol,nlev)        ! melting of ice to rain
  real(r8) :: nmelt_rain(mgncol,nlev)        ! melting of  ice number to rain
  real(r8), intent(out) :: rimo(:,:)           ! riming intensity  
  real(r8) ::  nmelto(mgncol,nlev)        ! melting of cloud ice
...
!zx
  ...
  substepping: do it=1,iter
     ...
     ! zx1119   initialize ice property
     rim = 0._r8
     cap_i = 0._r8
     aa_i = 0._r8
     ba_i = 0._r8
...
!        call size_dist_param_snow(qiic(:,k), niic(:,k), lami(:,k), n0i(:,k))
!        call size_dist_param_snow(qiic(:,k), niic(:,k), lams(:,k), n0s(:,k))
!--------------------------zx1119 add----------------------------------
...
!zx0131 add density factor to unrimed ice
        where (rim(:,k)<0.1_r8)
          ums(:,k)=ums(:,k)*rhof(:,k)  
          uns(:,k)=uns(:,k)*rhof(:,k) 
        end where 
        ...
        call accrete_cloud_water_snow(t(:,k), rho(:,k), av_i(:,k), uns(:,k), mu(:,k), &
             qcic(:,k), ncic(:,k), qiic(:,k), pgam(:,k), lamc(:,k), lami(:,k), n0i(:,k), &
             psacws(:,k), npsacws(:,k), bv_i(:,k),aa_i(:,k), ba_i(:,k))
...
!        end if
        ...
        call evaporate_sublimate_precip(deltat, t(:,k), p(:,k), rho(:,k), &
             ...
             qric(:,k), qiic(:,k), lamr(:,k), n0r(:,k), lami(:,k), n0i(:,k), cmei(:,k), &
             pre(:,k), prds(:,k))

!        call bergeron_process(t(:,k), rho(:,k), dv(:,k), mu(:,k), sc(:,k), &
!             qvl(:,k), qvi(:,k), asn(:,k), qcic(:,k), qiic(:,k), lami(:,k), n0i(:,k), &
!             bergs(:,k))
        bergs(:,k)=0._r8
...
!           ! conservation of qv
!           !-------------------------------------------------------------------
!           if (cmei(i,k).gt.0._r8) then
!                cmei(i,k)=min(cmei(i,k),q(i,k))
!           end if            
           ...
           if ((nprc(i,k)*lcldm(i,k)+(-nnuccr(i,k)+nsubr(i,k)-npracs(i,k)&
              ...
              !-------------------------------------------------------------------
           if (do_cldice) then
              dum = (mnuccc(i,k)+mnucct(i,k)+msacwi(i,k))*lcldm(i,k)+&
                    (bergs(i,k)+psacws(i,k))*lcldm(i,k)+&
                    (pracs(i,k)+mnuccr(i,k)+prds(i,k))*cldmax(i,k)+cmei(i,k)+berg(i,k)

              if (dum.lt.-qi(i,k)/deltat) then
                if (-prds(i,k).ge.qsmall) then
                if (-cmei(i,k).ge.qsmall) then
                  ratio = (qi(i,k)/deltat+dum-(prds(i,k)*cldmax(i,k)+cmei(i,k)))/&
                          (-prds(i,k)*cldmax(i,k)-cmei(i,k))*omsm
                  prds(i,k) = prds(i,k)*ratio
                  cmei(i,k)=cmei(i,k)*ratio
                else
                  ratio = (qi(i,k)/deltat+dum-prds(i,k)*cldmax(i,k))/(-prds(i,k)*cldmax(i,k))*omsm
                  prds(i,k) = prds(i,k)*ratio
                end if
                else
                  prds(i,k) = 0._r8
              ...
              nsubs(i,k)=0._r8
              !-------------------------------------------------------------------
              dum = ((-nnucct(i,k)-nsacwi(i,k))*lcldm(i,k)+(-nsubi(i,k))*icldm(i,k)&
                    -(nnuccr(i,k)+nsubs(i,k)+nsagg(i,k))*cldmax(i,k))*deltat

              if (dum.gt.nie) then
!zx160518
                  if ((nsubs(i,k)+nsagg(i,k)+nsubi(i,k)).ge.qsmall) then

                 ratio = (nie/deltat+(nnucct(i,k)+nsacwi(i,k))*lcldm(i,k)+nnuccr(i,k)*cldmax(i,k))/ &  
                      ((-nsubs(i,k)-nsagg(i,k))*cldmax(i,k)+(-nsubi(i,k))*icldm(i,k))*omsm
           ...
           prain(i,k) = prain(i,k)+(pra(i,k)+prc(i,k))*lcldm(i,k)+(-pracs(i,k)- &
                mnuccr(i,k))*cldmax(i,k)
           prodsnow(i,k) = prodsnow(i,k)+(mnuccc(i,k)+mnucct(i,k)+msacwi(i,k))*lcldm(i,k)+&
                           (psacws(i,k)+bergs(i,k))*lcldm(i,k)+&
                          (pracs(i,k)+mnuccr(i,k)+prds(i,k))*cldmax(i,k)+cmei(i,k)+berg(i,k)
           ...
           if (do_cldice) then
              nitend(i,k) = nitend(i,k)+ nnuccd(i,k)+ &
                   (nnucct(i,k)+nsacwi(i,k))*lcldm(i,k)+(nsubi(i,k))*icldm(i,k)&
                   +(nnuccr(i,k)+nsubs(i,k)+nsagg(i,k))*cldmax(i,k)
           end if
        ...
        do i = 1, mgncol
           ...
           prect(i) = prect(i)+(qrtend(i,k)*dz(i,k)*rho(i,k))/rhow
...
!           call size_dist_param_snow(qiic(i,k), niic(i,k), lams(i,k), n0s(i,k))
!-----------------------zx1119 add------------------
            call size_dist_param_ice_zx(qiic(i,k), niic(i,k), lami(i,k), n0i(i,k),am_i(i,k), bm_i(i,k),bm1(i,k))
...
!zx0131 add density factor to unrimed ice
            call ice_terminal_velocity(rim(i,k),av_i(i,k), bmbv1(i,k), &
                lami(i,k), bv_i(i,k), bm1(i,k), rhof(i,k), bv1(i,k),ums(i,k),uns(i,k))
...
!zx1119
        call ice_characteristics(nlev, mu(i,k), t(i,k), rho(i,k), dumc(i,k), &
             dumi(i,k), rimtot(i,k), cap_i(i,k), aa_i(i,k), ba_i(i,k), am_i(i,k), &
             bm_i(i,k),av_i(i,k),bv_i(i,k), bmbv1(i,k),bm1(i,k),bv1(i,k))
        call size_dist_param_ice_zx(dumi(i,k), dumni(i,k), lami(i,k), n0i(i,k), am_i(i,k), bm_i(i,k),bm1(i,k))
  ...
  !zx0131 add density factor to unrimed ice
            call ice_terminal_velocity(rimtot(i,k),av_i(i,k), bmbv1(i,k), &
                lami(i,k), bv_i(i,k), bm1(i,k), rhof(i,k), bv1(i,k),ums(i,k),uns(i,k))
            if (lami(i,k) .le. 0._r8) then
              ums(i,k)=0._r8
              uns(i,k)=0._r8
            end if

           vtrmi(i,k)=ums(i,k)
           fi(k) = g*rho(i,k)*ums(i,k)
           fni(k) = g*rho(i,k)*min(uns(i,k),1.2_r8)
        else
           fi(k) = 0._r8
           fni(k)= 0._r8
        end if
...
!just change the qrout(grid mean),put it after the do k=1.lev
                 ...
                 qrtot(i)=qrtot(i)+dum*dumi(i,k)*dz(i,k)*rho(i,k)/deltat
                 nrtot(i)=nrtot(i)+dum*dumni(i,k)*dz(i,k)*rho(i,k)/deltat
                 prect(i) = prect(i)+(dum*dumi(i,k)/deltat*dz(i,k)*rho(i,k))/rhow
!----------------------------end melt to rain-------------------------------
                 ...
                 nmelttot(i,k)=dum*dumni(i,k)/deltat
                 melt_rain(i,k)=melttot(i,k)
                 nmelt_rain(i,k)=nmelttot(i,k)
              end if
           end if
  ...
  do i = 1,mgncol
     do k=1,nlev
!0603        if (qc(i,k).ge.qsmall) then
        if (qc(i,k).ge.qsmall .and. (nc(i,k)+nctend(i,k)*deltat).ge.qsmall) then
...
elemental subroutine ice_characteristics(nlev, mu, t, rho, qcic, qiic, ri,&
  ...
  real(r8), parameter :: best_a=1.08 ,best_b = 0.499, g=9.18
...
!     psacws = min(psacws,qcic)
!     npsacws = min(npsacws,ncic)
  else
     psacws = 0._r8
     npsacws = 0._r8
  end if
